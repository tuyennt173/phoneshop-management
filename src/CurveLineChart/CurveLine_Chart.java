...
public class CurveLine_Chart extends JComponent {
    ...
    private void drawLabel(Graphics2D g2, SplinePoint s) {
        ...
        if (selectedIndex >= 0) {
            String text = blankPlotChart.getFormat().format(model.get(selectedIndex).getValue());
    ...
    private SplinePoint[] toPoint(Rectangle2D rec, int index, double maxValue) {
        ...
        for (int i = 0; i < model.size(); i++) {
            points[i + 1] = toPoint(rec, i, model.size(), model.get(i).getValue(), maxValue);
        }
        points[0] = points[1];
